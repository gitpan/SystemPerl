#$Id: SystemPerl.pod,v 1.11 2001/06/21 21:09:09 wsnyder Exp $
######################################################################
#
...
AUTODECL indicates where interface declarations should be inserted.  This
is described under each other AUTO that needs AUTODECL.
...
=item /*AUTOTRACE(I<module>)*/

AUTOTRACE creates a routine to trace the ports and signals in the current
module, and then call the tracing routine on all submodules.  AUTODECL is
also required in the declaration of the module.  

Note because of a SystemC limitation, output ports cannot be traced.  Also
the hiearchy is not properly placed into the trace file; the hiearchy path
...
void ex_mod::trace (sc_trace_file *tf, const sc_string& prefix, int levels, int options=0) {
